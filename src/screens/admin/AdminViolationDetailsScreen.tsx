...
import { Violation, ComplaintStatus } from '../../types';
import { doc, onSnapshot } from 'firebase/firestore';
import { db } from '../../config/firebase';
import { updateViolationAdmin } from '../../services/firestoreService';

export const AdminViolationDetailsScreen: React.FC<any> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(docRef, (snap) => {
      if (snap.exists()) {
        const data = { id: snap.id, ...snap.data() } as Violation;
        setViolation(data);
        
        // Init form state if not edited yet
        if (!saving) {
          setStatus(data.status || 'pending');
          setCustomStatusText(data.customStatusText || '');
          setMunicipalityReply(data.municipalityReply || data.adminNote || '');
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateViolationAdmin(violation.id!, {
        status,
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoBox}>
          <Text style={styles.infoTitle}>{violation.title}</Text>
          <Text style={styles.infoDesc}>{violation.description}</Text>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>القسم:</Text>
            <Text style={styles.infoValue}>{violation.categoryLabel}</Text>
          </View>
          <TouchableOpacity 
          ...
          >
            ...
            <Text style={[styles.infoValue, violation.location?.latitude ? { color: COLORS.danger, textDecorationLine: 'underline' } : {}]}>
              {violation.location.address || 'غير محدد'}
            </Text>
          </TouchableOpacity>
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>المواطن:</Text>
            <Text style={styles.infoValue}>{violation.citizenName} ({violation.citizenPhone})</Text>
          </View>
        </View>
        ...
        <Text style={[styles.sectionTitle, { marginTop: 20 }]}>رد البلدية والرقابة</Text>
        <TextInput
        ...
        <View style={styles.switchRow}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.switchSub}>سيتمكن جميع المستخدمين من رؤية المخالفة والرد</Text>
          </View>
          <Switch
            value={showOnHome}
            onValueChange={setShowOnHome}
            trackColor={{ false: '#CBD5E1', true: '#FFE4E6' }}
            thumbColor={showOnHome ? COLORS.danger : '#F8FAFC'}
          />
        </View>
...
const styles = StyleSheet.create({
  ...
  statusOptionActive: { backgroundColor: COLORS.danger, borderColor: COLORS.danger },
  ...
  saveBtn: { backgroundColor: COLORS.danger, borderRadius: 12, paddingVertical: 16, alignItems: 'center' },
