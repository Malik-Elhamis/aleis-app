...
import { subscribeElectricitySchedule, addElectricitySchedule, deleteElectricitySchedule, updateElectricitySchedule } from '../../services/firestoreService';
...
export const AdminElectricitySchedulesScreen: React.FC = () => {
  const [schedules, setSchedules] = useState<ElectricityScheduleItem[]>([]);
  ...
  useEffect(() => {
    const unsub = subscribeElectricitySchedule((items) => {
      setSchedules(items);
    });
    return () => unsub();
  }, []);
  ...
  const openFormForEdit = (item: ElectricityScheduleItem) => {
    ...
    setExpectedReturnTime(item.expectedReturnTime || '');
    setExpectedDay(item.expectedDay || '');
  ...
  const handleSave = async () => {
    if (!neighborhood || !cutoffTime || !expectedReturnTime || !expectedDay) {
    ...
    try {
      if (editingId) {
        await updateElectricitySchedule(editingId, {
          neighborhood,
          cutoffTime,
          expectedReturnTime,
          expectedDay,
  ...
  const handleDelete = (id: string) => {
    Alert.alert('تأكيد الحذف', 'هل أنت متأكد من حذف هذا الجدول؟', [
      ...
      { text: 'حذف', style: 'destructive', onPress: async () => {
        try {
          await deleteElectricitySchedule(id);
        } catch (e) {
    ...
    return (
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
        <ScrollView contentContainerStyle={styles.formContainer}>
          <Text style={styles.formTitle}>{editingId ? 'تعديل جدول انقطاع الكهرباء' : 'إضافة جدول جديد'}</Text>
          ...
          <Text style={styles.label}>وقت الانقطاع (بداية القطع)</Text>
          <TextInput style={styles.input} value={cutoffTime} onChangeText={setCutoffTime} placeholder="مثال: 02:00 ظهراً" textAlign="right" />

          <Text style={styles.label}>وقت التوصيل المتوقع (عودة الكهرباء)</Text>
          <TextInput style={styles.input} value={expectedReturnTime} onChangeText={setExpectedReturnTime} placeholder="مثال: 06:00 مساءً" textAlign="right" />

          <Text style={styles.label}>اليوم المجدول</Text>
          <TextInput style={styles.input} value={expectedDay} onChangeText={setExpectedDay} placeholder="مثال: يوم الثلاثاء" textAlign="right" />
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.addBtn} onPress={openFormForAdd}>
          ...
          <Text style={styles.addBtnText}>إضافة جدول انقطاع</Text>
        </TouchableOpacity>
      </View>
        ...
        renderItem={({ item }) => (
          <View style={[styles.card, SHADOWS.small]}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.cardSub}>{item.expectedDay} | {item.cutoffTime} - {item.expectedReturnTime}</Text>
            </View>
